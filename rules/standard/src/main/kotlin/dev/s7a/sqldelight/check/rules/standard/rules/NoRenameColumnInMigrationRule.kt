 ...
 */
public class NoRenameColumnInMigrationRule : Rule {
    override val id: RuleId = RuleId("no-rename-column-in-migration")
    ...
    override fun run(
        ...
        content.sqlTokens(hashLineComments = true)
            .toList()
            .sqlStatements()
            .mapNotNull { statement -> statement.findAlterTableOperation("rename", "column") }
            .forEach { match ->
                reporter.report(
                    RuleDiagnostic(
                        severity = defaultSeverity,
                        message = "Avoid RENAME COLUMN in SQLDelight migrations because it can break live application compatibility.",
                        file = context.file,
