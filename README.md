...
sqldelight-check is pre-release. The latest release is `v0.2.1`.
...
```kotlin
plugins {
    id("dev.s7a.sqldelight.check") version "0.2.1"
}
```
...
The built-in rule sets currently include 150 rules:

- `sqldelight-check-rules-standard`: 121 dialect-independent rules for `.sq` and `.sqm` files.
